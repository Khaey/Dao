...
begin
  if v_actor is null or not (dao_private.owner(p_project_id) or dao_private.staff()) then
  ...
  insert into public.award_items(award_id,project_id,contractor_id,request_id,bid_item_id,agreed_millimes)
    values(p_award_id,p_project_id,p_contractor_id,p_request_id,p_bid_item_id,p_agreed_millimes) returning id into v_item;
