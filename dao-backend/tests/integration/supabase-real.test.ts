...
test('Supabase real integration configuration', async()=>{
  if (missing.length) { console.log(`SKIP real integration; missing: ${missing.join(', ')}`); return; }
  ...
  if (absent.length) { console.log(`SKIP fixture assertions; missing: ${absent.join(', ')}`); return; }
