...
export class ProjectService {
  ...
  async updateRequest(input: Record<string, unknown>) { const { data, error } = await this.db.rpc('update_project_request', { p_request_id: input.request_id, p_trade_id: input.trade_id, p_title: input.title, p_scope: input.scope, p_budget_millimes: input.budget_millimes ?? null }); if (error) throw error; return data; }
  async withdrawRequest(requestId: string) { const { data, error } = await this.db.rpc('withdraw_project_request', { p_request_id: requestId }); if (error) throw error; return data; }
