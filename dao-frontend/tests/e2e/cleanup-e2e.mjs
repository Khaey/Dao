...
for(const id of [...new Set(state.projects||[])]){for(const table of ['project_version_requests','project_request_versions','project_requests','project_versions','projects'])await api(`/rest/v1/${table}?project_id=eq.${id}`,{method:'DELETE',headers:{Prefer:'return=minimal'}});}
