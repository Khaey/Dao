...
for(const profileId of contractorProfileIds){for(const table of ['bid_documents','bid_items','bid_versions','bids'])await api(`/rest/v1/${table}?contractor_id=eq.${profileId}`,{method:'DELETE',headers:{Prefer:'return=minimal'}});}
...
for(const profileId of contractorProfileIds){for(const table of ['contractor_trades','contractor_service_areas','contractor_profiles'])await api(`/rest/v1/${table}?${table==='contractor_profiles'?'id':'contractor_id'}=eq.${profileId}`,{method:'DELETE',headers:{Prefer:'return=minimal'}});}
