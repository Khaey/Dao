...
test('Client → reviewer DAO → artisan → offre DEV', async ({ browser }, testInfo) => {
  ...
  await client.getByLabel('Intitulé').fill('Lot plomberie E2E');
  ...
  await reviewer.getByLabel('Commentaire de revue').fill('Validation finale E2E');
  await reviewer.getByRole('button', { name: 'Approuver' }).click();
  await expect(reviewer.getByText('Aucune revue en attente.')).toBeVisible();
  await reviewer.goto(`/app/dao/publications/new?project_id=${projectId}`);
  ...
  await expect(reviewer.getByText(projectTitle)).toBeVisible();
  await reviewer.screenshot({ path: `test-results/${testInfo.project.name}-published.png`, fullPage: true });
