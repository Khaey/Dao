...
test('Client → reviewer DAO → artisan → offre DEV', async ({ browser }, testInfo) => {
  ...
  await expect(reviewer.getByText(projectTitle)).toBeVisible();
  await reviewer.getByLabel('Commentaire de revue').fill('Commentaire E2E DAO');
  await reviewer.getByRole('button', { name: 'Passer en revue DAO' }).click();
  await expect(reviewer.getByText('dao_review')).toBeVisible();
  await reviewer.getByLabel('Commentaire de revue').fill('À corriger : précisez le périmètre du lot.');
  await reviewer.getByRole('button', { name: 'Refuser' }).click();
  await expect(reviewer.getByText('Aucune revue en attente.')).toBeVisible();
  ...
  await finalReviewer.getByLabel('Commentaire de revue').fill('Version corrigée conforme.');
  await finalReviewer.getByRole('button', { name: 'Passer en revue DAO' }).click();
  await expect(finalReviewer.getByText('dao_review')).toBeVisible();
  await finalReviewer.getByLabel('Commentaire de revue').fill('Validation finale E2E');
  await finalReviewer.getByRole('button', { name: 'Approuver' }).click();
  await expect(finalReviewer.getByText('Aucune revue en attente.')).toBeVisible();
