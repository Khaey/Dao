...
type Row = { id: string; status: string; project_type: string; surface_m2?: number | null; indicative_budget_millimes?: number | null; created_at: string; title?: string; location?: string; lots?: number };
...
export default function Dashboard() {
  ...
  useEffect(() => { void (async () => {
    ...
    const [{ data: user }, projects, versions, requests] = await Promise.all([
      ...
      supabase.from('project_versions').select('project_id,title,governorate_id,version_no').order('version_no', { ascending: false }),
    ...
    setRows((projects.data ?? []).map(project => ({ ...project, title: latest.get(project.id)?.title, lots: lotCounts.get(project.id) ?? 0 })));
    setLoading(false);
  })(); }, []);

  const stats = useMemo(() => ({ total: rows.length, drafts: rows.filter(row => row.status === 'draft').length, review: rows.filter(row => ['client_review', 'dao_review', 'rejected'].includes(row.status)).length, open: rows.filter(row => ['open', 'published'].includes(row.status)).length }), [rows]);
  const next = rows.find(row => row.status === 'draft') ?? rows.find(row => ['client_review', 'rejected'].includes(row.status));
  ...
  return <section className="space-y-8">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[['Projets', stats.total], ['Brouillons', stats.drafts], ['En revue', stats.review], ['Ouverts / publiés', stats.open]].map(([label, value]) => <Card key={label} className="p-4"><p className="text-xs font-semibold uppercase tracking-wide text-black/40">{label}</p><p className="mt-2 text-3xl font-bold">{value}</p><p className="mt-1 text-xs text-black/45">Mis à jour en temps réel</p></Card>)}</div>
    {next ? <Card className="border-teal/20 bg-teal/[0.04]"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><div className="flex items-center gap-2"><Sparkles size={16} className="text-teal" /><p className="text-xs font-bold uppercase tracking-wide text-teal">Prochaine action</p></div><h2 className="mt-2 text-lg font-bold">{next.status === 'rejected' ? 'Corriger le DAO' : next.lots ? 'Vérifier les lots de votre projet' : 'Commencer votre projet'}</h2><p className="mt-1 text-sm text-black/55">{next.title || typeLabels[next.project_type] || 'Votre projet'} · {next.lots ?? 0} lot{(next.lots ?? 0) > 1 ? 's' : ''}</p></div><Link href={`/app/projects/${next.id}`}><Button className="bg-teal">Continuer <ArrowRight size={16} className="ml-2" /></Button></Link></div></Card> : <Card className="border-dashed"><div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-sand"><FolderPlus size={22} className="text-teal" /></div><div><h2 className="font-bold">Votre espace est prêt</h2><p className="mt-1 text-sm text-black/55">Créez votre premier projet et structurez vos travaux lot par lot.</p></div><Link className="sm:ml-auto" href="/app/projects/new"><Button className="bg-clay">Créer un projet</Button></Link></div></Card>}
    <div><SectionHeading eyebrow="Suivi" title="Projets récents" description="Retrouvez rapidement les dossiers sur lesquels vous travaillez." action={<Link href="/app/projects" className="text-sm font-semibold text-teal">Voir tous les projets <ArrowRight size={15} className="ml-1 inline" /></Link>} />{rows.length === 0 ? <p className="mt-5 text-sm text-black/50">Aucun projet pour le moment.</p> : <div className="mt-4 grid gap-3 lg:grid-cols-3">{rows.slice(0, 6).map(row => <Link href={`/app/projects/${row.id}`} key={row.id}><Card className="h-full p-4 transition hover:-translate-y-0.5 hover:border-teal/30"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold text-teal">{typeLabels[row.project_type] ?? 'Projet'}</p><h3 className="mt-1 line-clamp-1 font-bold">{row.title || 'Projet sans titre'}</h3></div><Badge tone={row.status === 'rejected' ? 'red' : row.status === 'open' ? 'teal' : 'neutral'}>{statusLabel[row.status] ?? row.status}</Badge></div><div className="mt-5 flex items-center justify-between text-xs text-black/50"><span>{row.lots ?? 0} lot{(row.lots ?? 0) > 1 ? 's' : ''}</span><span>{formatTnd(row.indicative_budget_millimes)}</span></div></Card></Link>)}</div>}</div>
  </section>;
}
