...
function NewPublication() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const [requestResult, versionResult, contractorResult] = await Promise.all([
        supabase.from('project_requests').select('id').eq('project_id', projectId).eq('status', 'open'),
        supabase.from('project_request_versions').select('request_id,title,scope,trade_id,version_no').eq('project_id', projectId).order('version_no', { ascending: false }),
      ...
      (versionResult.data ?? []).forEach(item => { if (!current.has(item.request_id)) current.set(item.request_id, item); });
      const lots = (requestResult.data ?? []).map(item => ({ ...item, ...current.get(item.id) }));
      setRows(lots); setSelected(lots.map(item => item.id)); setContractors(contractorResult.data ?? []);
      if (requestResult.error || versionResult.error) setError('Impossible de charger les lots du DAO.');
    })();
  }, [projectId]);
